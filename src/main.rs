...
fn main() -> Result<()> {
    ...
    let header = DatabaseHeader {
        ...
        number_of_pages: u32::from_be_bytes(header[28..32].try_into().unwrap()),
        text_encoding: match u32::from_be_bytes(header[56..60].try_into().unwrap()) {
            1 => TextEncoding::Utf8,
            _ => panic!("only utf8 is supported"),
        },
    };
